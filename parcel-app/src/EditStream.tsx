...
import { useCreateListener, useListener } from "./useListeners";
...
const EditStream: FC = () => {
  ...
  const { id } = useParams();
  //we need to load the params
  const { listener, refresh, loading, update } = useListener(parseInt(id || "0"));
  const { setTitle } = useBase();
  useEffect(() => {
    setTitle(listener?.name || "Edit Stream");
  }, [setTitle, listener]);
  if (loading) return <div>Loading...</div>;
  if (!listener) return <div>Listener not found</div>;
  return (
    <Formik
      initialValues={{
        name: listener.name,
      ...
      onSubmit={async (values, form) => {
        try {
          ...
          toast.success("Updated the FileStream!");
          navigate("/");
        } catch (e) {
          toast.dismiss(id);
          console.log(e);
